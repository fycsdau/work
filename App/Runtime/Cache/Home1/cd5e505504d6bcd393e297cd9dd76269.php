            ...
            <div id="bjui-collapse<?php echo ($c["id"]); ?>" class="panel-collapse collapse" style="height: 0px;">        
                <div class="panel-body">        
                    <ul class="menu-items" data-faicon="hand-o-up">
                        <?php $cate1=M('menu')->where('level=1 and status=1 and parent_id='.$c['id'])->order('sort')->select(); ?>
                        <?php if(is_array($cate1)): foreach($cate1 as $key=>$v): if(authcheck('Home/'.$v[alink],session('uid'))): $pid = $v['id']; $child_menu=M('menu')->where("level=2 and status=1 and parent_id='$pid'")->count(id); ?>
                                <?php if($child_menu == 0 ): ?><li>
                                    ...
                                    <li class="switch">
                                        ...
                                        <ul class="menu-items-children">
                                            <?php $cate2=M('menu')->where('level=2 and status=1 and parent_id='.$v['id'])->order('sort')->select(); ?>
